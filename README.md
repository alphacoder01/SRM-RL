...
**Scaling to more GPUs (speed vs. batch):** because `num_conditions` is per GPU, adding GPUs without changing it *grows the global batch* at the same seconds-per-iteration — it does not make an iteration faster. To make each iteration faster, keep the global batch fixed and shard it: set `rl.rollout.num_conditions = target_global / num_gpus` (groups must stay on one GPU, so the minimum is `num_conditions=1`). Example: to keep 16 global conditions, use `num_conditions=4` on 4 GPUs or `num_conditions=2` on 8 GPUs — each roughly halves per-iteration wall-clock vs. the step before. The startup log prints the effective global batch so you can confirm which regime you are in.

If you hit CUDA OOM, the dominant knob is `rl.update.update_batch_size` (default 16): the update retains activations for **backward**, so it costs far more memory per sample than the `no_grad` rollout — keep it ≤16 and lower to 8 if needed (it only affects throughput, not the optimization). The rollout batch (`rl.rollout.num_conditions × rl.rollout.group_size`) is comparatively cheap since it runs under `no_grad`; reduce `num_conditions` mainly for speed, not memory. `train_rl.sh` sets `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` to limit fragmentation; if OOM persists, also try `rl.rollout.compile=false` (compilation adds peak memory). Keep `rl.rollout.storage_device=cpu` (the default).
